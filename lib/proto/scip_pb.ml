...
let rec decode_metadata d =
  ...
  while !continue__ do
    ...
    | Some (1, Pbrt.Varint) -> begin
      v.version <- decode_protocol_version d;
    end
    | Some (1, pk) -> 
    ...
    | Some (3, Pbrt.Bytes) -> begin
      v.project_root <- Pbrt.Decoder.string d;
    end
    | Some (3, pk) -> 
...
let rec decode_occurrence d =
  ...
  while !continue__ do
    ...
    | Some (1, Pbrt.Bytes) -> begin
      v.range <- Pbrt.Decoder.packed_fold (fun l d -> (Pbrt.Decoder.int32_as_varint d)::l) [] d;
    end
    | Some (1, pk) -> 
    ...
    | Some (3, Pbrt.Varint) -> begin
      v.symbol_roles <- Pbrt.Decoder.int32_as_varint d;
    end
    | Some (3, pk) -> 
    ...
    | Some (5, Pbrt.Varint) -> begin
      v.syntax_kind <- decode_syntax_kind d;
    end
    | Some (5, pk) -> 
...
let rec decode_document d =
  ...
  while !continue__ do
    ...
    | Some (4, Pbrt.Bytes) -> begin
      v.language <- Pbrt.Decoder.string d;
    end
    | Some (4, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(document), field(4)" pk
    | Some (1, Pbrt.Bytes) -> begin
      v.relative_path <- Pbrt.Decoder.string d;
    end
    | Some (1, pk) -> 
...
let rec decode_index d =
  ...
  while !continue__ do
    ...
    | Some (3, Pbrt.Bytes) -> begin
      v.external_symbols <- (decode_symbol_information (Pbrt.Decoder.nested d)) :: v.external_symbols;
    end
    | Some (3, pk) -> 
...
let rec decode_package d =
  ...
  while !continue__ do
    ...
    | Some (1, Pbrt.Bytes) -> begin
      v.manager <- Pbrt.Decoder.string d;
    end
    | Some (1, pk) -> 
...
let rec decode_descriptor d =
  ...
  while !continue__ do
    ...
    | Some (2, Pbrt.Bytes) -> begin
      v.disambiguator <- Pbrt.Decoder.string d;
    end
    | Some (2, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(descriptor), field(2)" pk
    | Some (3, Pbrt.Varint) -> begin
      v.suffix <- decode_descriptor_suffix d;
    end
    | Some (3, pk) -> 
...
let rec decode_symbol d =
  ...
  while !continue__ do
    ...
    | Some (1, Pbrt.Bytes) -> begin
      v.scheme <- Pbrt.Decoder.string d;
    end
    | Some (1, pk) -> 
...
let rec encode_metadata (v:Scip_types.metadata) encoder = 
  ...
  begin match v.Scip_types.tool_info with
  | Some x -> 
    Pbrt.Encoder.key (2, Pbrt.Bytes) encoder; 
...
let rec encode_index (v:Scip_types.index) encoder = 
  begin match v.Scip_types.metadata with
  | Some x -> 
    Pbrt.Encoder.key (1, Pbrt.Bytes) encoder; 
...
let rec encode_symbol (v:Scip_types.symbol) encoder = 
  ...
  begin match v.Scip_types.package with
  | Some x -> 
    Pbrt.Encoder.key (2, Pbrt.Bytes) encoder; 
