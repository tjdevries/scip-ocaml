...
module CmFile = struct
  ...
  let of_string s =
    if Filename.check_suffix s ".cmt" then Cmt s
    else if Filename.check_suffix s ".cmti" then Cmti s
    else failwith "CmFile.of_string"

  let to_string = function Cmt s -> s | Cmti s -> s
...
let find_cm_files dir =
  let is_directory dir = try Sys.is_directory dir with Sys_error _ -> false in
