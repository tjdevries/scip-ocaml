...
let attribute sub x =
  let iterator =
    { Ast_iterator.default_iterator with location = (fun _this x -> sub.location sub x) }
  in
...
let attributes sub l = List.iter ~f:(attribute sub) l

let structure sub { str_items; str_final_env; _ } =
  List.iter ~f:(sub.structure_item sub) str_items;
...
let class_infos sub f x =
  ...
  List.iter ~f:(fun (ct, _) -> sub.typ sub ct) x.ci_params;
  f x.ci_expr
;;
...
let module_type_declaration sub x =
  ...
  Option.iter ~f:(sub.module_type sub) x.mtd_type
;;
...
let class_declaration sub x = class_infos sub (sub.class_expr sub) x
...
let structure_item sub { str_loc; str_desc; str_env; _ } =
  ...
  | Tstr_recmodule list -> List.iter ~f:(sub.module_binding sub) list
  | Tstr_modtype x -> sub.module_type_declaration sub x
  | Tstr_class list -> List.iter ~f:(fun (cls, _) -> sub.class_declaration sub cls) list
  | Tstr_class_type list ->
    List.iter
      ~f:(fun (_, s, cltd) ->
        iter_loc sub s;
        sub.class_type_declaration sub cltd)
      list
...
let constructor_decl sub x =
  ...
  List.iter ~f:(iter_loc sub) x.cd_vars;
  constructor_args sub x.cd_args;
  Option.iter ~f:(sub.typ sub) x.cd_res
;;
...
let type_declaration sub x =
  ...
  Option.iter ~f:(sub.typ sub) x.typ_manifest;
  List.iter ~f:(fun (c, _) -> sub.typ sub c) x.typ_params
;;

let type_declarations sub (_, list) = List.iter ~f:(sub.type_declaration sub) list

let type_extension sub x =
  sub.location sub x.tyext_loc;
  sub.attributes sub x.tyext_attributes;
  iter_loc sub x.tyext_txt;
  List.iter ~f:(fun (c, _) -> sub.typ sub c) x.tyext_params;
  List.iter ~f:(sub.extension_constructor sub) x.tyext_constructors
;;
...
let extension_constructor sub { ext_loc; ext_name; ext_kind; ext_attributes; _ } =
  ...
  | Text_decl (ids, ctl, cto) ->
    List.iter ~f:(iter_loc sub) ids;
    constructor_args sub ctl;
    Option.iter ~f:(sub.typ sub) cto
...
let pat : type k. iterator -> k general_pattern -> unit =
 fun sub { pat_loc; pat_extra = extra; pat_desc; pat_env; pat_attributes; _ } ->
  ...
  sub.env sub pat_env;
  List.iter ~f:(pat_extra sub) extra;
  match pat_desc with
  | Tpat_any -> ()
  | Tpat_var (_, s) -> iter_loc sub s
  | Tpat_constant _ -> ()
  | Tpat_tuple l -> List.iter ~f:(sub.pat sub) l
  | Tpat_construct (lid, _, l, vto) ->
    iter_loc sub lid;
    List.iter ~f:(sub.pat sub) l;
    Option.iter
      ~f:(fun (ids, ct) ->
        List.iter ~f:(iter_loc sub) ids;
        sub.typ sub ct)
      vto
  | Tpat_variant (_, po, _) -> Option.iter ~f:(sub.pat sub) po
  | Tpat_record (l, _) ->
    List.iter
      ~f:(fun (lid, _, i) ->
        iter_loc sub lid;
        sub.pat sub i)
      l
  | Tpat_array l -> List.iter ~f:(sub.pat sub) l
  | Tpat_alias (p, _, s) ->
    sub.pat sub p;
    iter_loc sub s
...
let expr sub { exp_loc; exp_extra; exp_desc; exp_env; exp_attributes; _ } =
  let extra = function
    ...
    | Texp_coerce (cty1, cty2) ->
      Option.iter ~f:(sub.typ sub) cty1;
      sub.typ sub cty2
    | Texp_newtype _ -> ()
    | Texp_poly cto -> Option.iter ~f:(sub.typ sub) cto
  in
  ...
  | Texp_let (rec_flag, list, exp) ->
    ...
    sub.expr sub exp
  | Texp_function { cases; _ } -> List.iter ~f:(sub.case sub) cases
  | Texp_apply (exp, list) ->
    sub.expr sub exp;
    List.iter ~f:(fun (_, o) -> Option.iter ~f:(sub.expr sub) o) list
  | Texp_match (exp, cases, _) ->
    sub.expr sub exp;
    List.iter ~f:(sub.case sub) cases
  | Texp_try (exp, cases) ->
    sub.expr sub exp;
    List.iter ~f:(sub.case sub) cases
  | Texp_tuple list -> List.iter ~f:(sub.expr sub) list
  | Texp_construct (lid, _, args) ->
    iter_loc sub lid;
    List.iter ~f:(sub.expr sub) args
  | Texp_variant (_, expo) -> Option.iter ~f:(sub.expr sub) expo
    ...
    Array.iter
      ~f:(function
        ...
        | _, Overridden (lid, exp) ->
          ...
          sub.expr sub exp)
      fields;
    Option.iter ~f:(sub.expr sub) extended_expression
  ...
  | Texp_setfield (exp1, lid, _, exp2) ->
    ...
    sub.expr sub exp1;
    sub.expr sub exp2
  | Texp_array list -> List.iter ~f:(sub.expr sub) list
  | Texp_ifthenelse (exp1, exp2, expo) ->
    sub.expr sub exp1;
    sub.expr sub exp2;
    Option.iter ~f:(sub.expr sub) expo
  ...
  | Texp_letop { let_ = l; ands; body; _ } ->
    ...
    List.iter ~f:(sub.binding_op sub) ands;
    sub.case sub body
  | Texp_unreachable -> ()
...
let signature sub { sig_items; sig_final_env; _ } =
  ...
  List.iter ~f:(sub.signature_item sub) sig_items
;;
...
let signature_item sub { sig_loc; sig_desc; sig_env; _ } =
  ...
  | Tsig_recmodule list -> List.iter ~f:(sub.module_declaration sub) list
  ...
  | Tsig_class list -> List.iter ~f:(sub.class_description sub) list
  | Tsig_class_type list -> List.iter ~f:(sub.class_type_declaration sub) list
...
let module_coercion sub = function
  ...
  | Tcoerce_structure (l1, l2) ->
    List.iter ~f:(fun (_, c) -> sub.module_coercion sub c) l1;
    List.iter ~f:(fun (_, _, c) -> sub.module_coercion sub c) l2
...
let module_expr sub { mod_loc; mod_desc; mod_env; mod_attributes; _ } =
  ...
  (* | Tmod_apply_unit mexp1 -> sub.module_expr sub mexp1 *)
...
let class_expr sub { cl_loc; cl_desc; cl_env; cl_attributes; _ } =
  ...
  | Tcl_constraint (cl, clty, _, _, _) ->
    ...
    Option.iter ~f:(sub.class_type sub) clty
  | Tcl_structure clstr -> sub.class_structure sub clstr
  | Tcl_fun (_, pat, priv, cl, _) ->
    sub.pat sub pat;
    List.iter ~f:(fun (_, e) -> sub.expr sub e) priv;
    sub.class_expr sub cl
  | Tcl_apply (cl, args) ->
    sub.class_expr sub cl;
    List.iter ~f:(fun (_, e) -> Option.iter ~f:(sub.expr sub) e) args
  | Tcl_let (rec_flag, value_bindings, ivars, cl) ->
    sub.value_bindings sub (rec_flag, value_bindings);
    List.iter ~f:(fun (_, e) -> sub.expr sub e) ivars;
    sub.class_expr sub cl
  | Tcl_ident (_, lid, tyl) ->
    iter_loc sub lid;
    List.iter ~f:(sub.typ sub) tyl
  | Tcl_open (od, e) ->
...
let class_signature sub { csig_self; csig_fields; _ } =
  ...
  List.iter ~f:(sub.class_type_field sub) csig_fields
;;
...
let typ sub { ctyp_loc; ctyp_desc; ctyp_env; ctyp_attributes; _ } =
  ...
  | Ttyp_arrow (_, ct1, ct2) ->
    sub.typ sub ct1;
    sub.typ sub ct2
  | Ttyp_tuple list -> List.iter ~f:(sub.typ sub) list
  | Ttyp_constr (_, lid, list) ->
    iter_loc sub lid;
    List.iter ~f:(sub.typ sub) list
  | Ttyp_object (list, _) -> List.iter ~f:(sub.object_field sub) list
  | Ttyp_class (_, lid, list) ->
    iter_loc sub lid;
    List.iter ~f:(sub.typ sub) list
  | Ttyp_alias (ct, _) -> sub.typ sub ct
  | Ttyp_variant (list, _, _) -> List.iter ~f:(sub.row_field sub) list
...
let class_structure sub { cstr_self; cstr_fields; _ } =
  ...
  List.iter ~f:(sub.class_field sub) cstr_fields
;;
...
let value_bindings sub (_, list) = List.iter ~f:(sub.value_binding sub) list

let case sub { c_lhs; c_guard; c_rhs } =
  sub.pat sub c_lhs;
  Option.iter ~f:(sub.expr sub) c_guard;
  sub.expr sub c_rhs
;;
...
let handle_structure _ structure =
  let iter =
    { iter with
      location =
        (fun _ _ ->
          (*  TODO: Feels pretty bad to have to check if we've visited a location
                before... This may be a bug? Or perhaps some other invariant i didn't expect.
                But this is fine for now. Hopefully this doesn't get too large of a map.

                Should be OK since it's only per-structure. *)
          (* let range = ScipRange.of_loc loc in *)
          (* visited := Map.set visited.contents ~key ~data:range; *)
          (* match IndexSymbols.lookup index_lookup loc with *)
          (* | Some symbol -> add_occurence (default_occurrence ~range ~symbol ()) *)
          (* | None -> ()) *)
          assert false)
    }
  in
