...
let lparen = char '('
let rparen = char ')'
let letter = function 'a' .. 'z' -> true | 'A' .. 'Z' -> true | _ -> false
...
let ident =
  choice ~failure_msg:"could not get ident" [ escaped_ident; simple_ident ]
...
let type_parameter_desc =
  to_descriptor Type_parameter @@ (lbrace *> ident) <* rbrace

let this_desc =
  choice ~failure_msg:"Failed to find descriptor"
    [
      term_desc;
      namespace_desc;
      type_desc;
...
let space = char ' '
let space_escaped = take_while1 (function ' ' -> false | _ -> true) <* space
